    ...
    public String getObjectIdsQuery(@Nullable String filters, @Nullable String mainCondition) {
        ...
        if (!TextUtils.isEmpty(filters)) {
            if (!TextUtils.isEmpty(mainCondition)) {
                String sql =
                        "SELECT ec_client_search.object_id, ec_client_search.last_interacted_with, (opd_details.current_visit_start_date IS NOT NULL AND opd_details.current_visit_start_date >= '$latest_start_visit_date' AND opd_details.current_visit_end_date IS NULL) AS checked_in FROM ec_client_search WHERE ec_client_search.date_removed IS NULL AND phrase MATCH '%s*') " +
                        "    LEFT JOIN opd_details ON ec_client_search.object_id = opd_details.base_entity_id" +
                                ...
                                " WHERE crt.register_type = 'opd' AND checked_in " +
